...
class DeepSort(object):
    ...
    def update_tracks(self, raw_detections, embeds=None, frame=None, today=None, others=None, instance_masks=None):
        ...
        if frame is not None:
            # Remove tracks that are out of frame
            frame_height, frame_width = frame.shape[:2]  # Assuming 'frame' is a numpy array with shape (H, W, C)
            self.tracker.tracks = [
                track for track in self.tracker.tracks
                # Top left corner
                if (0 <= track.to_tlwh()[0] <= frame_width and 0 <= track.to_tlwh()[1] <= frame_height)
                # Top right corner
                or (0 <= track.to_tlwh()[0] + track.to_tlwh()[2] <= frame_width and 0 <= track.to_tlwh()[1] <= frame_height)
            ]
