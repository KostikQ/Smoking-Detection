...
class Tracker:
    ...
    def update(self, detections, today=None):
        ...
        for track_idx, detection_idx in matches:
            ...
            # Compute the Euclidean distance between track and detection bbox centers.
            track_center = [track.to_tlwh()[0] + track.to_tlwh()[2] / 2, track.to_tlwh()[1] + track.to_tlwh()[3] / 2]
            detection_center = [detection.ltwh[0] + detection.ltwh[2] / 2, detection.ltwh[1] + detection.ltwh[3] / 2]
            distance = np.linalg.norm(np.array(track_center) - np.array(detection_center))
    
            # If the distance is larger than the threshold, mark the track as missed and initiate a new track for the detection.
